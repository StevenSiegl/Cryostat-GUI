...
Classes:
    DSP7265_Updater: a class for interfacing with a signal recovery DSP 7265 Lock-In Amplifier
            inherits from AbstractLoopThread
                there, the looping behaviour of this thread is defined
            uses the pymeasure DSP7265 command set
Author(s):
...
from pymeasure.signalrecovery.dsp7265 import DSP7265

from util import AbstractLoopThread
from util import ExceptionHandling


class DSP7265_Updater(AbstractLoopThread):
    """Updater class to update all instrument data of the DSP 7265

    NOT NECESSARILY FUNCTIONAL!!

    """

    def __init__(self, InstrumentAddress='', internal_res, **kwargs):
        """init: get the driver connection to the Lock-In, set up default conf"""
        super().__init__(**kwargs)

        self.lockin = DSP7265(InstrumentAddress)
        self.__name__ = 'DSP7265_Updater ' + InstrumentAddress
    ...
    def running(self):
        ...
        SampleCurrent_A = data['Voltage_V'] / \
            (self.ShuntResistance_Ohm + self.ContactResistance_Ohm + self.internal_res)
    ...
    def setVoltage(self):
        """set a voltage"""
        self.lockin.oltage = self.set_Voltage_V
